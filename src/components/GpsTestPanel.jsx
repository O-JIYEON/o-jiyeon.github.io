  ...
  return (
    <aside ref={panelRef} className="gps-test-panel" aria-label="GPS 테스트 결과 패널">
      ...
      <section className="gps-session-list-section" aria-label="GPS 세션 목록">
        <div className="gps-session-list-section__header">
          ...
          <div className="gps-session-list-section__actions">
            ...
            <button
              type="button"
              className="gps-session-refresh-button"
              onClick={onRefresh}
              disabled={gpsSessionsLoading}
            >
              새로고침
            </button>
          </div>
        ...
        {gpsSessionsError ? (
          <p className="gps-session-list__status gps-session-list__status--error">{gpsSessionsError}</p>
        ) : null}
      ...
      <section className="gps-result-section" aria-label="GPS 테스트 결과">
        ...
        {gpsTrackError ? (
          <p className="gps-session-list__status gps-session-list__status--error">{gpsTrackError}</p>
        ) : null}
